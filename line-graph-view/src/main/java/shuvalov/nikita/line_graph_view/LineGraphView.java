...
//==================================== Builder Set Variables ==============
    ...
    private LineGraphView(Context context, List<Float> values, int fillColor, int lineColor) {
        super(context);
    ...
    private void preparePaints() {
        ...
        if(mFillColor >0){
            ...
            mGoalPaint.setColor(mFillColor);
        }

        mProgressPaint = new Paint();
        mProgressPaint.setStyle(Paint.Style.FILL);
        mProgressPaint.setColor(mFillColor);
    ...
    private void definePaths(){
        ...
        double maxPointValue =  Collections.max(mValues);
        float xIntervalSize = mLineGraphRect.width()/ (float)(mValues.size()-1);
        float yPerPixel = (float)(mLineGraphRect.height()/(maxPointValue * 1.1f));
        mGoalPath = new Path();
...
//        int cursorWeek = startWeek;
        ...
        Log.d("definePaths:", "Height: " +mLineGraphRect.height());
        int numOfValues = mValues.size();
        for(int i = 0; i < numOfValues; i++){
            float value = mValues.get(i);
            Log.d(TAG, "definePaths: "+ yPerPixel * value);
            Log.d(TAG, "definePaths: " + xIntervalSize*i);
            mProgressPath.lineTo(mLineGraphRect.left + xIntervalSize * i, mLineGraphRect.bottom - yPerPixel * value);
//
//
...
//        int startWeek = mValues.get(0).getWeekNum();
...
//    }
    ...
    public static class LineGraphViewBuilder{
        ...
        public LineGraphView build(Context context) {
            return new LineGraphView(context, mValues, mFillColor, mLineColor);
        }
    }
}
